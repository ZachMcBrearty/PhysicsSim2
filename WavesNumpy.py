    ...
    if setupStage == 0:
        if key == b"1":
            ...
            v = Vector3DField(x=width/10, y=height/10, width=width*4/5, 
                              height=height*4/5, density=5, wavespeed=10)
            setupStage = 1
        else:
            print(f"key {key} not recognised")
...
if __name__ == "__main__":
    # v = Vector3DField(x=width/10, y=height/10, width=width*4/5, 
    # height=height*4/5, density=5, wavespeed=10)
    # #perturbStanding3D(v, 2, 2, 500)
    # perturbLocal3D(v, 40, 40, 25, 25, 1000)
    #v = Vector2DField(x=width/10, y=height/2, width=width*4/5, density=1, wavespeed=5) 
    #perturbStanding2DPos(v, 5, 50)
    #perturbStanding2DVel(v, 4, 25)
    #s = lambda n, wid, i: sin(radians(n*180/wid*i))  
    #perturbLocal2DPos(v, int(floor(v.w/2)), int(floor(v.w/5)), 2, 100, s)
    #c = lambda n, wid, i: cos(radians(n*180/wid*i)) 
    #perturbLocal2DVel(v, int(floor(v.w/2)), int(floor(v.w/5)), 2, 55.5, c)
    #perturbLocal2DVel(v, int(floor(v.w/2)), int(floor(v.w/5)), 2, 100)
    #perturbLocal2DVel(v, int(floor(v.w/2)), int(floor(v.w/10)), -3, 100)
    #for x in range(1, 10):
    #    perturbStanding2DPos(v, 2*x+1, 2.5*x)
    #gaussian2DPos(v, int(floor(v.w/2)), int(floor(v.w/20)), 200)

    #gaussian2DPos(v, int(floor(v.w/2)), int(floor(v.w/20)), 200)
    #f = lambda x, wid, p: -(p-x)*exp(-((p-x)/wid)**2 / 2) / wid**2
    #func2DVel(v, int(floor(v.w/2)), int(floor(v.w/20)), 1500, f)
    # gaussian2DPos(v, int(floor(v.w/2-v.w/20)), int(floor(v.w/20)), -200)

    #gaussian2DVel(v, int(floor(v.w/2+v.w/20)), int(floor(v.w/20)), +50)
    #gaussian2DVel(v, int(ceil(v.w/2-v.w/20)), int(floor(v.w/20)), -50)
