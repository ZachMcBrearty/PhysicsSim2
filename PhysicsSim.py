...
G = 1.5           # m^3 kg^-1 s^-2
Gdot = 0.05       # m^3 kg^-1 s^-3, sinusoidal variation
stepsPerFrame = 0 # 
timestep = 0.1    # s
t = 0             # s
arrows = True
...
class Planet(Circle):
    ...
    def fromorbit(cls, name, rad, ownmass, parent, colour, semimajoraxis, aphelion, angle):
        pos = Vector.frompolar(semimajoraxis, angle)
    ...
    def interact(self, objects):
        for _ in range(stepsPerFrame):
            ...
            for x in objects:
                if self != x:
                    d = self.dist(x)
                    self.acc -= d.normalise() * (G - Gdot*sin(radians(t))) * x.mass / (abs(d)**2)
...
def RealValueSystem():
    ...
    lengthScale = 0.07 / Sun["Radius"]

    system = [Planet(Sun["Name"], 1, Sun["Mass"] * massScale, Sun["Colour"], width/2, height/2)]

    for obj in Planets:
        p = Planet.fromorbit(
            obj["Name"],                       # Name
            obj["Radius"] * lengthScale,       # Radius
        ...
        a = obj["Semimajor"] * lengthScale, obj["Aphelion"] * lengthScale
        system.append(p)
...
# system.append(Planet.fromorbit("Alpha", 
#                                 2.5, 
#                                 0.1, 
#                                 system[0], 
#                                 (200, 200, 0), 
